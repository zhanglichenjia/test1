...
$(function () {
    ...
    jxmstc.browserForEasyui(function (obj) {
        ...
        var toUrl=getRootPath()+"/admin/web/discount/discount.jsp?type=2&id="+obj.id;
        layer.open({
            title:'折扣信息浏览',
            type: 2,
            area:["100%","100%"],
    ...
    jxmstc.editForEasyui(function (obj) {
        var toUrl=getRootPath()+"/admin/web/discount/discount.jsp?type=1&id="+obj.id;
        layer.open({
            title:'折扣信息修改',
            type: 2,
            area:["100%","100%"],
    ...
    jxmstc.delForEasyui("您确定删除当前选的折扣吗？",function (data) {
        if (data.code>0){
            layer.msg("删除成功！");
            jxmstc.findForEasyui("dgDiscount",getCondition());
        }else
    ...
    $("#btnAdd").click(function () {
        var toUrl=getRootPath()+"/admin/web/discount/discount.jsp?type=0";
        layer.open({
            title:'折扣信息新增',
            type: 2,
            area:["100%","100%"],
    ...
    $("#btnDeleteAll").click(function () {
        ...
        jxmstc.do("deleteAll",obj,function (data) {
            if (data.code>0)
            {
                layer.msg("您选中的折扣已经删除！");
                find();
            }
            else
            {
                layer.msg("您选中的折扣删除失败，错误代码："+data.code);
            }
        });
    });
