...
<form class="layui-form" id="ff">
    ...
    <br/>
    <div class="layui-form-item">
        <label class="layui-form-label">未打折的价格： </label>
        <div class="layui-input-block"style="width:500px;">
            <input name="oldPrice" required style="width:500px;" lay-verify="required" autocomplete="off" placeholder="请输入未打折的价格" class="layui-input" type="text">
        </div>
    </div>
    ...
    <div class="layui-form-item">
        <label class="layui-form-label">打折后的价格： </label>
        <div class="layui-input-block"style="width:500px;">
            <input name="nowPrice" required style="width:500px;" lay-verify="required" autocomplete="off" placeholder="请输入打折后的价格" class="layui-input" type="text">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">总价： </label>
        <div class="layui-input-block"style="width:500px;">
            <input name="sumPrice" required style="width:500px;" lay-verify="required" autocomplete="off" placeholder="请输入总价" class="layui-input" type="text">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">对应的订单流水号（唯一）： </label>
        <div class="layui-input-block"style="width:500px;">
            <select name="orders.id" id="sltWeb1"></select>
        </div>
    </div>
    ...
    $(function () {
        ...
        doData("orders/find",null,function (data) {
            if (data!=null && data.obj!=undefined && data.obj!=null)
                for(var i=0;i<data.obj.length;i++)
                {
                    $("#sltWeb1").append("<option value='"+data.obj[i].id+"'>"+data.obj[i].name+"</option>");
                }
            if(type!=0)
                kb.getMe(getURLParamValue("id"),function (data) {
                    $('#ff').form('load', data);//导入修改的数据，并绑定在界面上

                    $("#sltWeb1").find("option[value='"+data.orders.id+"']").attr("selected","selected");
